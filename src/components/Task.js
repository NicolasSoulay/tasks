const Task = (props) => {
    return (
        <section className="border border-2 border-secondary p-2 my-2 d-flex align-items-center justify-content-between">
            ...
            <p className="my-0">debut: {props.readableDate(props.task.created)}</p>
            <p className="my-0">fin: {props.task.ended}</p>
            <div>
                <button 
                    onClick ={(event) => {
                ...
                {!props.task.isValidate && <button 
                    onClick = {(event) => {
                        props.handleClickShowUpdateForm(props.task.id, props.task.label, props.task.description, props.task.date);
                    }}
                className="btn btn-primary me-1">Mettre à jour</button>}
                <button 
                    onClick ={() => {
